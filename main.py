# =============================================================================
# SIMPLE_ROUNDTRIP_PREDICTION.py
# - Single-record default dummy input
# - MAIN hanya berisi input & output
# - All other logic in functions
# - Final output: list of objects with only 'product' and 'predicted_sales'
# =============================================================================
...
def get_warung_info_from_input_single_default():
    n = input_int("Berapa banyak baris warung yang ingin dimasukkan? (ketik 0 untuk gunakan 1 dummy) ", default=0)
    if n == 0:
        return pd.DataFrame({'store_nbr': ['toko_a'], 'type': ['D']})
    rows = []
    for i in range(n):
        store = input(f"store_nbr #{i+1}: ").strip()
        typ = input(f"type #{i+1} (ex A/B/C/D): ").strip()
        rows.append({'store_nbr': store, 'type': typ})
    return pd.DataFrame(rows)

def get_laporan_harian_from_input_single_default(default_date):
    n = input_int("Berapa banyak record laporan yang ingin dimasukkan? (ketik 0 untuk gunakan 1 dummy) ", default=0)
    if n == 0:
        return pd.DataFrame({
    ...
    rows = []
    for i in range(n):
        date_in = input(f"date #{i+1} (YYYY-MM-DD) [enter = {default_date.strftime('%Y-%m-%d')}]: ").strip()
        if date_in == "":
            d = pd.to_datetime(default_date)
        else:
            try:
                d = pd.to_datetime(date_in)
            except:
                print("Format tanggal salah, gunakan default.")
                d = pd.to_datetime(default_date)
        store = input(f"store_nbr #{i+1}: ").strip()
        prod = input(f"nama_produk #{i+1}: ").strip()
        qty_str = input(f"jumlah_terjual #{i+1} (angka): ").strip()
        try:
            qty = float(qty_str)
        except:
            qty = 0.0
        rows.append({'date': d, 'store_nbr': store, 'nama_produk': prod, 'jumlah_terjual': qty})
    return pd.DataFrame(rows)
...
# ----------------------------
#  Presence extraction (store-level)
# ----------------------------
...
# ----------------------------
#  Main pipeline (returns aggregated product-level list of dicts with only product+predicted_sales)
# ----------------------------
def run_prediction_and_return_product_list(historical_data, laporan_harian, warung_info,
...
if __name__ == "__main__":
    ...
    warung_info_input = get_warung_info_from_input_single_default()
    laporan_harian_input = get_laporan_harian_from_input_single_default(default_date=today_simulation)

    output = run_prediction_and_return_product_list(
        historical_data=hist_df,
        laporan_harian=laporan_harian_input,
        warung_info=warung_info_input,
